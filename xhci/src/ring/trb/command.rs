...
allowed! {
    /// TRB which are allowed to be pushed to the Command Ring
    enum {
        /// Link TRB
        Link,
        ...
        StopEndpoint,
        /// Set RT Dequeue Pointer Command TRB
        SetTrDequeuePointer,
...
impl AddressDevice {
    /// Input Context Pointer fieldに値をセットする
    /// # Panic
    /// このメソッドは`p`が16-byteアラインされていない場合パニックします
    ...
    /// Input Context Pointer fieldの値を返す
    #[must_use]
...
impl ConfigureEndpoint {
    /// Input Context Pointer fieldに値をセットします
    /// # Panics
    /// このメソッドは`p`が16-byteにアラインされていない時パニックします
    pub fn set_input_context_pointer(&mut self, p: u64) -> &mut Self {
        assert_eq!(
            p % 16,
            0,
...
impl EvaluateContext {
    /// Input Context Pointer fieldに値をセットします
    /// # Panics
    /// このメソッドは`p`が16-byteアラインされていない時パニックします
    pub fn set_input_context_pointer(&mut self, p: u64) -> &mut Self {
        assert_eq!(
            p % 16,
            0,
...
impl ForceEvent {
    /// Event TRB Pointer fieldに値をセットする
    /// # Panics
    /// このメソッドは`p`が16-byteアラインされていない時パニックします
    pub fn set_vent_trb_pointer(&mut self, p: u64) -> &mut Self {
    ...
    rw_field!([3](22..=31), vf_id, "VF ID", u8);
}
...
impl GetPortBandwidth {
    /// Port Bandwidth Context Pointerに値をセットします
    /// # Panics
    /// このメソッドは`p`が16-byteアラインされていない時パニックします
    ...
    /// Port Bandwidth Context Pointerの値を返す
    #[must_use]
...
impl ForceHeader {
    ...
    /// Head Info fieldに値をセットします
    /// # Panics
    /// このメソッドは`info[0]`の下位5ビットが0でない時パニックします
    pub fn set_handler_info(&mut self, info: [u32; 3])-> &mut Self {
        assert!(
...
impl GetExtendedProperty {
    /// Extended Property Context Pointerに値をセットします
    /// # Panics
    /// このメソッドは`p`が16-byteアラインされていない時パニックします
    ...
    /// Extended Property Context Pointerの値を返す
    #[must_use]
...
impl SetExtendedProperty {
    ...
    rw_field!([3](16..=18), capability_parameter, "Capability Parameter", u8);
    rw_field!([3](16..=18), command_sub_type, "Command Sub Type", u8);
    rw_field!([3](19..=23), endpoint_id, "Endpoint ID", u8);
    rw_field!([3](24..=31), slot_id, "Slot ID", u8);
}
