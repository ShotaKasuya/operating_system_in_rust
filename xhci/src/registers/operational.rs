...
use crate::registers::capability::{Capability, CapabilityRegistersLength};
use accessor::{single, Mapper, array};
    ...
    M: Mapper + Clone,
{
    ...
    pub dncrtl: single::ReadWrite<DeviceNotificationControl, M>,
...
where
    M: Mapper + Clone,
{
    /// Host Control Operational Registerへの新しいアクセッサを作ります
    /// # Safety
    /// この構造体を通してのみアクセスされることを保証しなければならない
    /// # Panics
    /// このメソッドは引数が指すアドレスが正しくアラインされていない時パニックします
...
impl UsbCommandRegister {
    ...
    rw_bit!(10, enable_werap_event, "Enable Wrap Event");
...
impl UsbStatusRegister {
    ...
    ro_bit!(2, host_system_error, "Host System Error");
    ro_bit!(3, event_interrupt, "Event Interrupt");
    ro_bit!(4, port_change_detect, "Port Change Detect");
    ro_bit!(8, save_state_status, "Save State Status");
    ro_bit!(9, restore_state_status, "Restore State Status");
    ro_bit!(10, save_restore_error, "Save/Restore Error");
...
impl DeviceNotificationControl {
    /// Notification Enableフィールドのi番目の値を返す(0 index)
    ///
    /// # Panic
    /// `i >= 16`でパニック
    #[must_use]
    pub fn get(self, i: usize) -> bool {
        Self::ensure_is_within_range(i);

        self.0.get_bit(i)
    }

    /// Notification Enableフィールドのi番目にセットする(0 index)
    ///
    /// # Panic
    /// `i >= 16`でパニック
    ...
    /// Notification Enableフィールドのi番目をクリアする(0 index)
    ///
    /// # Panic
    /// `i >= 16`でパニック
    ...
    fn ensure_is_within_range(i: usize) {
        assert!(
            i < 16,
...
impl DeviceContextBaseAddressArrayPointerRegister {
    /// Device Context Base Address Array Pointerの値を返す
    #[must_use]
    pub fn get(self) -> u64 {
        self.0
    }

    /// Device Context Base Address Array Pointerに値をセットする
    ///
    /// # Panic
    /// 64byteにアラインされていない場合パニックする
    pub fn set(&mut self, p: u64) {
        assert!(
            p.trailing_zeros() >= 6,
            "The Device Context Base Address Array Pointer must be 64-bit aligned."
        );
...
impl PortRegisterSet {
    /// Port Register Setの配列への新しいアクセッサを作ります
    /// # Safety
    /// 呼び出し元からアクセッサが一度だけしか作られないことを保証する必要があります
    /// # Panics
    /// このメソッドは`base address`が正しくアラインメントされていない時パニックする
    ...
    M2: Mapper+Clone,
    {
        let base = mmio_base + usize::from(capability.caplength.read_volatile().get())+0x0400;
        array::ReadWrite::new(
            base,
            capability
                .hcsparams1
...
impl PortStatusAndControlRegister {
    ...
    rw1c_bit!(1, poty_enabled_disabled, "Port Enable/Disabled");
    ro_bit!(3, over_rurrent_active, "Over-current Active");
    ...
    ro_bit!(24, cold_attack_status, "Cold Attack Status");
...
impl PortPowerManagementStatusAndControlRegister {
    ...
    /// PortTestControlの値を返す
    /// 値が予約済みを示す場合、、`None`を返す
    #[must_use]
...
impl PortHardwareLpmControlRegister {
    rw_field!(
        0..=1,
        host_intitiated_resume_duration_mode,
...
impl From<PortIndicator> for u32 {
    fn from(value: PortIndicator) -> Self {
        value as _
    }
}
