    ...
    M: Mapper + Clone,
{
    ...
    pub hciverson: single::ReadOnly<InterfaceVersionNumber, M>,
    ...
    pub hccoarans1: single::ReadOnly<CapabilityParameters1, M>,
    /// Doorbell Offset
...
where
    M: Mapper + Clone,
{
    /// Crates a new accessor to the Host Controller Capability Registers.
    ///
    /// # Safety
    /// このメソッドの呼び出し元がこの構造体を通してのみアクセスされることを保証しなければならない
    ///
    /// # Panics
    /// このメソッドは`mmio_base`が正しくアラインメントされないときパニックします
...
impl InterfaceVersionNumber {
    /// HCでサポートされているxHCI仕様リビジョン番号のBCDエンコードを返す。
    /// 値の最上位バイトはメジャー番号を表し、再開バイトにはマイナーバージョンの拡張が含まれる。
    /// 例えば、0x0110はxHCIバージョン1.1.0を意味する
    #[must_use]
    pub fn get(self) -> u16 {
        self.0
    }
...
impl StructuralParameters2 {
    ...
    /// EventRingSegmentTableの容量を返す
    /// ERSTMaxには指数値が含まれていますが、このメソッドは計算済みの値を返します
    #[must_use]
    pub fn event_ring_segment_table_max(self) -> u16 {
        2_u16.pow(self.erst_max())
    }

    /// xHCに必要なスクラッチパッドの数を返す
    #[must_use]
...
impl CapabilityParameters2 {
    ro_bit!(0, u32_entry_capability, "U3 Entry Capability");
    ro_bit!(
        1,
    ...
    ro_bit!(6, extended_tbc_caapability, "Extended TBC Capability");
    ro_bit!(
        7,
...
impl_debug_from_methods! {
    CapabilityParameters2 {
        u32_entry_capability,
        configure_endpoint_command_max_exit_latency_too_large_capability,
        force_save_context_capability,
        compliance_transition_capability,
        large_esit_payload_capability,
        configuration_information_capability,
        extended_tbc_caapability,
        extended_tbc_trb_status_capability,
        get_set_extended_property_capability,
        virtualization_based_trusted_io_capability,
    }
