macro_rules! impl_debug_from_methods {
    ($name:ident {$($method:ident),*$(,)?}) => {
        ...
        f.debug_struct(core::stringify!($name))
                $(.field(core::stringify!($method), &self.$method()))*.finish()
    }
        }
    };
}
...
macro_rules! ro_bit {
    ...
    ($offset:literal, $method:ident, $name:literal) => {
        bit_getter!($offset, $method, $name);
    };
}
...
macro_rules! rw1c_bit {
    ([$offset:literal]($bit:literal),$method:ident, $name:literal) => {
        ...
        paste::paste! {
            ...
            #[doc=$name]
            #[doc="bit, preventing the bit from being cleard on write"]
